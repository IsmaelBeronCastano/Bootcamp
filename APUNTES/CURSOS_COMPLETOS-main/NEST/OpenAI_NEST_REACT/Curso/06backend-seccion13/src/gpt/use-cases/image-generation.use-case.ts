...
export const imageGenerationUseCase = async (
  ...
  if (!originalImage || !maskImage) {
    ...
    // Guardo la imagen en el FS
    const fileName = await downloadImageAsPng(response.data[0].url);
    const url = `${process.env.SERVER_URL}/gpt/image-generation/${fileName}`;

    return {
      url: url, //retorno la url http://localhost:3000....
  ...
  const response = await openai.images.edit({ //uso el método edit
    model: 'dall-e-2',
    prompt: prompt,
    image: fs.createReadStream(pngImagePath), //le paso la imagen por streaming
    mask: fs.createReadStream(maskPath),//le paso la máscara
